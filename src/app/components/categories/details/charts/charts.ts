...
@Component({
  selector: 'component-categories-charts',
  imports: [
    LucideChevronDown,
    LucidePlus,
    LucideChartColumn,
...
export class ComponentCategoriesCharts implements OnInit, OnChanges, AfterViewInit {
  ...
  private initializeChart() {
    const chartElement = document.querySelector('#hs-category-bar-chart') as HTMLElement;
  ...
  loadChartData() {
    this.categoryService.getCategoryCharts(this.categoryId).subscribe({
      next: (response) => {
      ...
      error: (err) => console.error('Error fetching category charts', err)
    });
  }
