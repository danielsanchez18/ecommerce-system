...
@Component({
  selector: 'component-billing-pdf',
  standalone: true,
  imports: [CommonModule, LucidePrinter, LucideX],
  templateUrl: './pdf.html',
})
export class ComponentBillingPdf {
  @Input() invoice!: InvoiceResponse | null;
  @Input() isOpen: boolean = false;
  @Output() close = new EventEmitter<void>();

  printReceipt() {
    const receiptElement = document.getElementById('invoice-content');
    ...
    Array.from(document.body.children).forEach((child: Element) => {
      ...
      if (
        ...
        htmlChild.id !== 'invoice-print-style'
      ) {
    ...
    style.innerHTML = `
      @media print {
        ...
        #invoice-content h1 { font-size: 16px !important; }
        #invoice-content p, #invoice-content span, #invoice-content td, #invoice-content th { font-size: 12px !important; }
      }
    `;
